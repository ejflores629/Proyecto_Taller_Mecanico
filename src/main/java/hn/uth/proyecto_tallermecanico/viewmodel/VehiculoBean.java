...
public class VehiculoBean implements Serializable {
    ...
    static {
        // Configuración de marcas y modelos (igual que antes)
        ...
        MODELOS_MOTO.put("Honda", Arrays.asList("CBR 600", "Navi", "Dio", "Africa Twin"));
    ...
    // CAMBIO IMPORTANTE: Usar update() y create() explícitos
    public void guardarVehiculo() {
        try {
            if (this.vehiculoSeleccionado != null && this.vehiculoSeleccionado.getPlaca() != null) {
                // EDICIÓN -> Llamar a update()
                vehiculoRepository.update(this.vehiculoSeleccionado);
                addMessage(FacesMessage.SEVERITY_INFO, "Actualizado", "Vehículo actualizado correctamente.");
                this.vehiculoSeleccionado = null;
            } else {
                // CREACIÓN -> Llamar a create()
                vehiculoRepository.create(this.vehiculoNuevo);
